...
public class LoggerManager {
    ...
    public static void changeOutputFile(String fileName) {
        ...
        Appender appender = config.getAppender("LogFile"); // Replace "MyFileAppender" with your appender's name

        if (appender instanceof FileAppender) {
            appender.stop(); // Stop the appender before making changes

            FileAppender fileAppender = (FileAppender) appender;
            FileAppender newFileAppender = FileAppender.newBuilder()
            ...
            ((org.apache.logging.log4j.core.Logger) LogManager.getRootLogger()).addAppender(newFileAppender);
            context.updateLoggers(); // Update the context to apply changes

            LoggerConfig loggerConfig = config.getLoggerConfig(LogManager.ROOT_LOGGER_NAME); // For root logger
