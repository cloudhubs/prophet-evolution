...
public class MicroserviceMetrics {
  ...
  private void generateNumericMetrics() {
    ...
    // TODO this probably needs to be changed, we dont only want to check old service. Also should
    // write threshold/how much it exceeds by
  ...
  public void generateSiucMetrics(
      Map<String, Microservice> oldMicroserviceMap, Map<String, Microservice> newMicroserviceMap) {
    this.oldSiucScore =
        oldMicroservice == null ? 0 : calculateSIUCScore(oldMicroserviceMap, oldMicroservice);
    this.newSidc2Score =
        newMicroservice == null ? 0 : calculateSIUCScore(newMicroserviceMap, newMicroservice);
  }
}
