...
public class EntityDepAssessor {
    ...
    public static void run(SystemChange systemChange, MsSystem msSystem) {
        ...
        for(Delta d : systemChange.getEntities()) {
            ...
            MsModel model = msSystem.getServiceMap().get(d.getMsName());
     ...
     *
     */
    private static boolean validateObjectChange(MsModel model, Delta delta) throws RuntimeException {
        Optional<JClass> oldClass = Stream.concat(model.getEntities().stream(), model.getDtos().stream()).filter(jClass -> jClass.getClassPath().equals(delta.getLocalPath())).findFirst();
