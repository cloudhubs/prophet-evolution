...
//                Microservice oldMicroservice = microserviceMap.values().stream().filter(ms -> ms.getId().equals(oldDependency.getService())).findFirst().orElse(null);
...
//    }
    ...
    private static JsonArray convertCallChangeToJsonArray(List<CallChange> callChangeList) {
        ...
        for (CallChange callChange : callChangeList) {
            ...
            jsonObjectBuilder.add("oldLink", Objects.isNull(callChange.getOldLink()) ? "" : callChange.getOldLink().getMsSource() + " -> " + callChange.getOldLink().getMsDestination());
            jsonObjectBuilder.add("newLink", Objects.isNull(callChange.getNewLink()) ? "" : callChange.getNewLink().getMsSource() + " -> " + callChange.getNewLink().getMsDestination());
    ...
    private static JsonArray convertEndpointChangeToJsonArray(List<EndpointChange> callChangeList) {
        ...
        for (EndpointChange endpointChange : callChangeList) {
            ...
            jsonObjectBuilder.add("oldEndpoint", JsonConvertUtils.buildMethod(endpointChange.getOldEndpoint()));
            jsonObjectBuilder.add("newEndpoint", JsonConvertUtils.buildMethod(endpointChange.getNewEndpoint()));
            jsonObjectBuilder.add("oldLinks", convertLinkListToJsonArray(endpointChange.getOldLinkList()));
            jsonObjectBuilder.add("newLinks", convertLinkListToJsonArray(endpointChange.getNewLinkList()));
    ...
    private void writePlaceholdersToFile(String fileName) throws IOException {
        ...
        for(Metric metric : metricList) {
            ...
            if(metric.getClassRole().equals(ClassRole.SERVICE)) {
                ...
                objectBuilder.add("EndpointChanges", convertEndpointChangeToJsonArray(metric.getEndpointChangeList()));
            }
    ...
    public List<EndpointChange> getAllEndpointChanges(Delta delta) {
        ...
        switch (delta.getChangeType()) {
            case ADD:
                ...
                break;
            case MODIFY:
                oldController = microserviceMap.get(delta.getMsName()).getControllers().stream().filter(c -> c.getClassPath().equals(delta.getLocalPath())).findFirst().orElse(null);
                endpointChanges.addAll(compareEndpoints(oldController.getEndpoints(), delta.getCChange().getEndpoints(), delta));
    ...
    public List<CallChange> getAllRestCallChanges(Delta delta) {
        ...
        switch (delta.getChangeType()) {
            case ADD:
                ...
                break;
            case MODIFY:
                oldService = microserviceMap.get(delta.getMsName()).getServices().stream().filter(s -> s.getClassPath().equals(delta.getLocalPath())).findFirst().orElse(null);
     ...
     * @return
     */
    private List<CallChange> compareRestCalls(JService oldService, JService newService, ChangeType changeType) {
        List<CallChange> callChanges = new ArrayList<>();
     ...
     * @return
     */
    private List<EndpointChange> compareEndpoints(List<Endpoint> oldEndpointList, List<Endpoint> newEndpointList, Delta delta) {
        List<EndpointChange> endpointChanges = new ArrayList<>();

        // Delete
        if(newEndpointList == null) {
            for (Endpoint oldEndpoint : oldEndpointList) {
                endpointChanges.add(new EndpointChange(oldEndpoint, null,
                        getEndpointLinks(oldEndpoint, delta.getMsName()), new ArrayList<>(), delta.getChangeType()));
        ...
        if(oldEndpointList == null) {
            for (Endpoint newEndpoint : newEndpointList) {
                endpointChanges.add(new EndpointChange(null, newEndpoint,
                        new ArrayList<>(), getEndpointLinks(newEndpoint, delta.getMsName()), delta.getChangeType()));
        ...
        for(Endpoint oldEndpoint : oldEndpointList) {
            for(Endpoint newEndpoint : newEndpointList) {
                if(oldEndpoint.getMethodName().equals(newEndpoint.getMethodName())) {
                    endpointChanges.add(new EndpointChange(oldEndpoint, newEndpoint,
                            getEndpointLinks(oldEndpoint, delta.getMsName()), getEndpointLinks(newEndpoint, delta.getMsName()), delta.getChangeType()));
                }
            }
        }
