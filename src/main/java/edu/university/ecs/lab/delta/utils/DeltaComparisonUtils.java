...
public class DeltaComparisonUtils {
  ...
  //  public JsonObject extractDeltaAdditionChanges(Map<String, MsModel> currentModelMap, String
  // repoPath, String pathToLocal) {
  ...
  //
  //    Optional<MsModel> model = currentModelMap.values().stream().filter(msModel ->
  // msModel.getMsPath().equals(repoPath)).findFirst();
  //
  ...
  //      Optional<JController> controller =
  // model.get().getControllers().stream().filter(jController ->
  // jController.getClassName().equals(jClass.getClassName())).findFirst();
  //
  //      // We have a new class? Add it all...
  //      if(controller.isEmpty()) {
  //        jsonObjectBuilder.add("controllers", JsonConvertUtils.buildRestControllers("",
  // List.of((JController) jClass)));
  //      } else {
  //        // We parse what specifically changed
  //        JController controllerChanges = (JController) extractClassSpecificChanges(jClass,
  // controller.get());
  //        jsonObjectBuilder.add("controllers", JsonConvertUtils.buildRestControllers("",
  // List.of(controllerChanges)));
  //      }
  //
  //    } else if (jClass instanceof JService) {
  //      Optional<JService> service = model.get().getServices().stream().filter(jService ->
  // jService.getClassName().equals(jClass.getClassName())).findFirst();
  //
  //
  //      // We have a new class? Add it all...
  //      if(service.isEmpty()) {
  //        jsonObjectBuilder.add("services", JsonConvertUtils.buildRestServices(List.of((JService)
  // jClass)));
  //      } else {
  //        // We parse what specifically changed
  //        JService serviceChanges = (JService) extractClassSpecificChanges(jClass, service.get());
  //        jsonObjectBuilder.add("services",
  // JsonConvertUtils.buildRestServices(List.of(serviceChanges)));
  //      }
  //    } else {
  //      // Search remaining class types for a match
  //      Optional<JClass> dto = model.get().getDtos().stream().filter(jMatch ->
  // jMatch.getClassName().equals(jClass.getClassName())).findFirst();
  //      Optional<JClass> repository = model.get().getRepositories().stream().filter(jMatch ->
  // jMatch.getClassName().equals(jClass.getClassName())).findFirst();
  //      Optional<JClass> entity = model.get().getEntities().stream().filter(jMatch ->
  // jMatch.getClassName().equals(jClass.getClassName())).findFirst();
  //
  ...
  //          jsonObjectBuilder.add("repositories",
  // JsonConvertUtils.buildJavaClass(List.of(jClass)));
  //        }
  //
  ...
  //        JClass classChanges = extractClassSpecificChanges(jClass, dto.get());
  //        jsonObjectBuilder.add("repositories",
  // JsonConvertUtils.buildJavaClass(List.of(classChanges)));
  //      } else {
  //        JClass classChanges = extractClassSpecificChanges(jClass, dto.get());
  //        jsonObjectBuilder.add("entities",
  // JsonConvertUtils.buildJavaClass(List.of(classChanges)));
  //      }
  //
  //    }
  ...
  //    returnClass.setMethods(extractListSpecificChanges(jClass1.getMethods(),
  // jClass2.getMethods()));
  //    returnClass.setMethodCalls(extractListSpecificChanges(jClass1.getMethodCalls(),
  // jClass2.getMethodCalls()));
  //
  //    if(jClass1 instanceof JController && jClass2 instanceof JController) {
  //      JController controller = (JController) returnClass;
  //      controller.setEndpoints(extractListSpecificChanges(((JController)jClass1).getEndpoints(),
  // ((JController)jClass2).getEndpoints()));
  //      return controller;
  //    } else if(jClass1 instanceof JService && jClass2 instanceof JService) {
  //      JService service = (JService) returnClass;
  //      service.setRestCalls(extractListSpecificChanges(((JService)jClass1).getRestCalls(),
  // ((JService)jClass2).getRestCalls()));
  //      return service;
  //    }
  //
  //
