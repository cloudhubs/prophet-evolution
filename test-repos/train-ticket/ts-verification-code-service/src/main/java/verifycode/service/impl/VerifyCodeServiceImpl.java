...
public class VerifyCodeServiceImpl implements VerifyCodeService {
  ...
  /** build local cache */
  public Cache<String, String> cacheCode =
  ...
  public Map<String, Object> getImageCode(
    ...
    String cookieId;
    if (cookie == null) {
      VerifyCodeServiceImpl.LOGGER.warn(
          "[getImageCode][Get image code warn.Cookie not found][Path Info: {}]",
          request.getPathInfo());
  ...
  public boolean verifyCode(
    ...
    if (cookie == null) {
      VerifyCodeServiceImpl.LOGGER.warn(
          "[verifyCode][Verify code warn][Cookie not found][Path Info: {}]", request.getPathInfo());
      cookieId = UUID.randomUUID().toString().replace("-", "").toUpperCase();
      CookieUtil.addCookie(response, ysbCaptcha, cookieId, CAPTCHA_EXPIRED);
    } else {
      cookieId = cookie.getValue();
    ...
    if (code == null) {
      VerifyCodeServiceImpl.LOGGER.warn(
          "[verifyCode][Get image code warn][Code not found][CookieId: {}]", cookieId);
      return false;
    }
