...
public class PaymentServiceImpl implements PaymentService {
  ...
  public Response pay(Payment info, HttpHeaders headers) {
    ...
    if (paymentRepository.findByOrderId(info.getOrderId()) == null) {
      ...
      PaymentServiceImpl.LOGGER.warn(
          "[pay][Pay Failed][Order not found with order id][PaymentId: {}, OrderId: {}]",
          info.getId(),
          info.getOrderId());
      return new Response<>(
          0, "Pay Failed, order not found with order id" + info.getOrderId(), null);
    }
  }
  ...
  public void initPayment(Payment payment, HttpHeaders headers) {
    ...
    if (!paymentTemp.isPresent()) {
      paymentRepository.save(payment);
      PaymentServiceImpl.LOGGER.error(
          "[initPayment][Init payment error][Payment not found][PaymentId: {}]", payment.getId());
    } else {
      PaymentServiceImpl.LOGGER.info(
          "[initPayment][Init Payment Already Exists][PaymentId: {}]", payment.getId());
    }
  }
}
