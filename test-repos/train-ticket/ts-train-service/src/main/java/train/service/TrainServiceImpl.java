...
public class TrainServiceImpl implements TrainService {
  ...
  @Autowired private TrainTypeRepository repository;
  ...
  public boolean create(TrainType trainType, HttpHeaders headers) {
    ...
    if (repository.findByName(trainType.getName()) == null) {
      TrainType type =
          new TrainType(
              trainType.getName(), trainType.getEconomyClass(), trainType.getConfortClass());
      type.setAverageSpeed(trainType.getAverageSpeed());
      repository.save(type);
      result = true;
    } else {
      TrainServiceImpl.LOGGER.error(
          "[create][Create train error][Train already exists][TrainTypeId: {}]", trainType.getId());
    }
    return result;
  }
  ...
  public TrainType retrieve(String id, HttpHeaders headers) {
    if (!repository.findById(id).isPresent()) {
      TrainServiceImpl.LOGGER.error(
          "[retrieve][Retrieve train error][Train not found][TrainTypeId: {}]", id);
      return null;
    } else {
  ...
  public TrainType retrieveByName(String name, HttpHeaders headers) {
    ...
    if (tt == null) {
      TrainServiceImpl.LOGGER.error(
          "[retrieveByName][RetrieveByName error][Train not found][TrainTypeName: {}]", name);
      return null;
    } else {
      return tt;
    }
  ...
  public List<TrainType> retrieveByNames(List<String> names, HttpHeaders headers) {
    ...
    if (tt == null || tt.isEmpty()) {
      TrainServiceImpl.LOGGER.error(
          "[retrieveByNames][RetrieveByNames error][Train not found][TrainTypeNames: {}]", names);
      return null;
    } else {
      return tt;
    }
  ...
  public boolean update(TrainType trainType, HttpHeaders headers) {
    ...
    if (repository.findById(trainType.getId()).isPresent()) {
      ...
      TrainServiceImpl.LOGGER.error(
          "[update][Update train error][Train not found][TrainTypeId: {}]", trainType.getId());
    }
    return result;
  }
  ...
  public boolean delete(String id, HttpHeaders headers) {
    ...
    if (repository.findById(id).isPresent()) {
      ...
      TrainServiceImpl.LOGGER.error(
          "[delete][Delete train error][Train not found][TrainTypeId: {}]", id);
    }
    return result;
  }
