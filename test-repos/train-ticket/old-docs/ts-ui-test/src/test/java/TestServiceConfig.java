...
public class TestServiceConfig {
  ...
  public void testConfig() throws Exception {
    ...
    js.executeScript(
        "document.getElementById('config_update_name').value='DirectTicketAllocationProportion'");
  ...
  public void testQueryConfig() throws Exception {
    ...
    Thread.sleep(1000);
    // gain Travel list
    List<WebElement> configList =
        driver.findElements(By.xpath("//table[@id='query_config_list_table']/tbody/tr"));
    if (configList.size() > 0)
      System.out.printf("Success to Query Config and Config list size is %d.%n", configList.size());
    else System.out.println("Failed to Query Config or Config list size is 0");
