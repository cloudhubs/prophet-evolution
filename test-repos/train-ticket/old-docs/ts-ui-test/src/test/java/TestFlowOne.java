...
public class TestFlowOne {
  ...
  public void testLogin() throws Exception {
    ...
    if ("".equals(statusLogin)) System.out.println("Failed to Login! Status is Null!");
    else if (statusLogin.startsWith("Success"))
      System.out.println("Success to Login! Status:" + statusLogin);
    else System.out.println("Failed to Login! Status:" + statusLogin);
  ...
  public void testBooking() throws Exception {
    ...
    WebElement elementBookingStartingPlace =
        driver.findElement(By.id("travel_booking_startingPlace"));
    elementBookingStartingPlace.clear();
    elementBookingStartingPlace.sendKeys("Shang Hai");

    // locate booking terminalPlace input
    WebElement elementBookingTerminalPlace =
        driver.findElement(By.id("travel_booking_terminalPlace"));
    ...
    List<WebElement> ticketsList =
        driver.findElements(By.xpath("//table[@id='tickets_booking_list_table']/tbody/tr"));
    ...
    if (ticketsList.size() > 0) {
      ...
      System.out.printf(
          "Success to search tickets，the tickets list size is:%d%n", ticketsList.size());
      ...
      Thread.sleep(1000);
    } else System.out.println("Tickets search failed!!!");
  ...
  public void testSelectContacts() throws Exception {
    List<WebElement> contactsList =
        driver.findElements(By.xpath("//table[@id='contacts_booking_list_table']/tbody/tr"));
    // Confirm ticket selection
    ...
    if (contactsList.size() == 0) System.out.println("Show Contacts failed!");
  ...
  public void testTicketPay() throws Exception {
    ...
    if (bStatusPay == false) System.out.println("Confirming Ticket failed!");
    Assert.assertEquals(bStatusPay, true);

    driver.findElement(By.id("preserve_pay_button")).click();
    Thread.sleep(1000);
    String itemCollectOrderId =
        driver.findElement(By.id("preserve_collect_order_id")).getAttribute("value");
  ...
  public void testTicketCollect() throws Exception {
    ...
    if (bCollectOrderId == false) System.out.println("Ticket payment failed!");
    Assert.assertEquals(bCollectOrderId, true);

    driver.findElement(By.id("preserve_collect_button")).click();
    Thread.sleep(1000);
    String statusCollectOrderId =
        driver.findElement(By.id("preserve_collect_order_status")).getText();
    ...
    else System.out.println("Failed to Collect Ticket! Status is:" + statusCollectOrderId);
  ...
  public void testEnterStation() throws Exception {
    String itemEnterOrderId =
        driver.findElement(By.id("preserve_execute_order_id")).getAttribute("value");
    if ("".equals(itemEnterOrderId)) System.out.println("Enter Station,No Order Id,failed");
    ...
    else System.out.println("Failed to Enter Station! Status is:" + statusEnterStation);
