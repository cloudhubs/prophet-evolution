...
public class TestServiceBasicInfo {
  ...
  public void testBasicInfo() throws Exception {
    ...
    js.executeScript(
        "document.getElementById('basic_information_startingStation').value='shanghai'");
    js.executeScript("document.getElementById('basic_information_stations').value='beijing'");
    js.executeScript(
        "document.getElementById('basic_information_terminalStation').value='taiyuan'");

    String jsstartingTime =
        "document.getElementById('basic_information_startingTime').value='09:51'";
    ...
    js.executeScript(
        "document.getElementById('basic_information_startingPlace').value='Shang Hai'");
    ...
    js.executeScript(
        "document.getElementById('basic_information_departureTime').value='" + bookDate + "'");

    driver.findElement(By.id("basic_information_button")).click();
    Thread.sleep(1000);

    // gain BasicInfo list
    List<WebElement> basicInfoList =
        driver.findElements(By.xpath("//table[@id='query_basic_information_list_table']/tbody/tr"));
    if (basicInfoList.size() > 0)
      System.out.printf(
          "Success to Query BasicInfo and BasicInfo list size is %d.%n", basicInfoList.size());
    else System.out.println("Failed to Query BasicInfo or BasicInfo list size is 0");
