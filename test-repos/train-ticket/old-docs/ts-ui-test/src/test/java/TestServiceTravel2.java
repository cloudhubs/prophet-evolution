...
public class TestServiceTravel2 {
  ...
  public void testTravel2() throws Exception {
    ...
    js.executeScript(
        "document.getElementById('travel2_update_startingStationId').value='shanghai'");
  ...
  public void testQueryTravel2() throws Exception {
    ...
    Thread.sleep(1000);
    // gain Travel list
    List<WebElement> travel2List =
        driver.findElements(By.xpath("//table[@id='query_travel2_list_table']/tbody/tr"));

    if (travel2List.size() > 0)
      System.out.printf(
          "Success to Query Travel2 and Travel2 list size is %d.%n", travel2List.size());
    else System.out.println("Failed to Query Travel2 or Travel2 list size is 0");
