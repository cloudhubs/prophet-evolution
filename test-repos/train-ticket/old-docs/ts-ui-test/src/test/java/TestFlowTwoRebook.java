...
public class TestFlowTwoRebook {
  ...
  public void testViewOrders() throws Exception {
    ...
    if (myOrdersList.size() > 0) {
      System.out.printf(
          "Success to show my orders list，the list size is:%d%n", myOrdersList.size());
    } else
      System.out.println(
          "Failed to show my orders list，the list size is 0 or No orders in this user!");
  ...
  public void testChangeOrder() throws Exception {
    ...
    for (i = 0; i < myOrdersList.size(); i++) {
      statusOrder =
          myOrdersList
              .get(i)
              .findElement(By.xpath("div[2]//form[@role='form']/div[7]/div/label[2]"))
              .getText();
      if (statusOrder.startsWith("Paid")) break;
    }
    ...
    myOrdersList
        .get(i)
        .findElement(By.xpath("div[2]//form[@role='form']/div[12]/div/button[1]"))
        .click();
    Thread.sleep(1000);
    String inputStartingPlace =
        driver.findElement(By.id("travel_rebook_startingPlace")).getAttribute("value");
    String inputTerminalPlace =
        driver.findElement(By.id("travel_rebook_terminalPlace")).getAttribute("value");
    ...
    if (bchangeStatus == false) System.out.println("Step-Change Your Order,The input is null!!");
    ...
    changeTicketsSearchList =
        driver.findElements(By.xpath("//table[@id='tickets_change_list_table']/tbody/tr"));
    if (changeTicketsSearchList.size() > 0) {
      System.out.printf(
          "Success to search tickets，the tickets list size is:%d%n",
          changeTicketsSearchList.size());
    } else
      System.out.println(
          "Failed to search tickets，the tickets list size is 0 or No tickets available!");
  ...
  public void testTicketRebook() throws Exception {
    ...
    WebElement elementBookingSeat =
        changeTicketsSearchList.get(0).findElement(By.xpath("td[10]/select"));
    ...
    String itemSeatType =
        driver.findElement(By.id("ticket_rebook_confirm_seatType_String")).getText();
    ...
    if ("".equals(statusAlert)) {
      ...
      if (bPrice == false) System.out.println("Confirming Ticket failed!");
    ...
    } else System.out.println("Failed,Rebook status:" + statusAlert);
  }
