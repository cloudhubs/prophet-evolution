...
public class TestServiceTicketInfo {
  ...
  public void testTicketInfo() throws Exception {
    ...
    js.executeScript(
        "document.getElementById('ticketinfo_departureTime').value='" + bookDate + "'");

    driver.findElement(By.id("ticketinfo_button")).click();
    Thread.sleep(1000);

    // gain TicketInfo list
    List<WebElement> ticketInfoList =
        driver.findElements(By.xpath("//table[@id='query_ticketinfo_list_table']/tbody/tr"));
    if (ticketInfoList.size() > 0)
      System.out.printf(
          "Success to Query TicketInfo and TicketInfo list size is %d.%n", ticketInfoList.size());
    else System.out.println("Failed to Query TicketInfo or TicketInfo list size is 0");
