...
public class TestServiceTravel {
  ...
  public void testTravel(TravelInfo travelinfo) throws Exception {
    ...
    driver
        .findElement(By.id("travel_update_startingStationId"))
        .sendKeys(travelinfo.startStationName);
    ...
    driver
        .findElement(By.id("travel_update_terminalStationId"))
        .sendKeys(travelinfo.terminalStationId);

    JavascriptExecutor js = (JavascriptExecutor) driver;
    String jsStartingTime =
        "document.getElementById('travel_update_startingTime').value='"
            + travelinfo.startingTime
            + "'";
    ...
    String jsEndTime =
        "document.getElementById('travel_update_endTime').value='" + travelinfo.endTime + "'";
  ...
  public void testQueryTravel() throws Exception {
    ...
    Thread.sleep(1000);
    // gain Travel list
    List<WebElement> travelList =
        driver.findElements(By.xpath("//table[@id='query_travel_list_table']/tbody/tr"));

    if (travelList.size() > 0)
      System.out.printf("Success to Query Travel and Travel list size is %d.%n", travelList.size());
    else System.out.println("Failed to Query Travel or Travel list size is 0");
