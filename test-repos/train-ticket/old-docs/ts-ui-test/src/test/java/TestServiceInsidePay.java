...
public class TestServiceInsidePay {
  ...
  public void testInsidePay() throws Exception {
    ...
    driver
        .findElement(By.id("inside_payment_orderId"))
        .sendKeys("5ad7750b-a68b-49c0-a8c0-32776b067703");
    ...
    else System.out.println("False, status of inside payment result is null!");
  ...
  public void testInsidePayList() throws Exception {
    ...
    List<WebElement> insidePayList =
        driver.findElements(
            By.xpath("//table[@id='query_inside_payment_payment_list_table']/tbody/tr"));
    if (insidePayList.size() > 0)
      System.out.printf(
          "Success to Query InsidePayList and InsidePay list size is %d.%n", insidePayList.size());
    else System.out.println("Failed to Query InsidePayList or InsidePay list size is 0");
  ...
  public void testUserBalance() throws Exception {
    ...
    List<WebElement> userBalanceList =
        driver.findElements(
            By.xpath("//table[@id='query_inside_payment_account_list_table']/tbody/tr"));
    if (userBalanceList.size() > 0)
      System.out.printf(
          "Success to Query UserBalanceList and UserBalanceList list size is %d.%n",
          userBalanceList.size());
    else System.out.println("Failed to Query UserBalanceList or UserBalanceList list size is 0");
  ...
  public void testAddMoney() throws Exception {
    ...
    List<WebElement> addMoneyList =
        driver.findElements(
            By.xpath("//table[@id='query_inside_payment_add_money_list_table']/tbody/tr"));
    if (addMoneyList.size() > 0)
      System.out.printf(
          "Success to Query Add Money List and Add Money List list size is %d.%n",
          addMoneyList.size());
    else System.out.println("Failed to Query Add Money List or Add Money List list size is 0");
