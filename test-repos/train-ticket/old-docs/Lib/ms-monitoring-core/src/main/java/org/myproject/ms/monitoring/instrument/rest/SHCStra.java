...
public class SHCStra extends HystrixConcurrencyStrategy {
  ...
  private static final Log log = LogFactory.getLog(SHCStra.class);
  ...
  private void logCurrentStateOfHysrixPlugins(
  ...
  public <T> Callable<T> wrapCallable(Callable<T> callable) {
    ...
    Callable<T> wrappedCallable =
        this.delegate != null ? this.delegate.wrapCallable(callable) : callable;
  ...
  static class HystrixTraceCallable<S> implements Callable<S> {
    ...
    public HystrixTraceCallable(Chainer tracer, ChainKeys traceKeys, Callable<S> callable) {
      this.tracer = tracer;
      this.traceKeys = traceKeys;
    ...
    public S call() throws Exception {
      ...
      String asyncKey =
          this.traceKeys.getAsync().getPrefix() + this.traceKeys.getAsync().getThreadNameKey();
