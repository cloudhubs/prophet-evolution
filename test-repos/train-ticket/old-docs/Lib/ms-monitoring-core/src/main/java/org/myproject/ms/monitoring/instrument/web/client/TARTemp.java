...
public class TARTemp extends AsyncRestTemplate {
  ...
  public TARTemp(AsyncClientHttpRequestFactory asyncRequestFactory, Chainer tracer) {
    super(asyncRequestFactory);
    this.tracer = tracer;
  }

  public TARTemp(
      AsyncClientHttpRequestFactory asyncRequestFactory,
  ...
  public TARTemp(
      AsyncClientHttpRequestFactory requestFactory, RestTemplate restTemplate, Chainer tracer) {
  ...
  protected <T> ListenableFuture<T> doExecute(
    ...
    return new ListenableFuture<T>() {
      ...
      public void addCallback(ListenableFutureCallback<? super T> callback) {
        future.addCallback(
            new TraceListenableFutureCallbackWrapper<>(TARTemp.this.tracer, span, callback));
      }
  ...
  private static class TraceSuccessCallback<T> implements SuccessCallback<T> {
    ...
    private TraceSuccessCallback(Chainer tracer, Item parent, SuccessCallback<T> delegate) {
      this.tracer = tracer;
      this.parent = parent;
  ...
  private static class TraceFailureCallback implements FailureCallback {
    ...
    private TraceFailureCallback(Chainer tracer, Item parent, FailureCallback delegate) {
      this.tracer = tracer;
      this.parent = parent;
      this.delegate = delegate;
    }
  ...
  private static class TraceListenableFutureCallbackWrapper<T>
    ...
    private TraceListenableFutureCallbackWrapper(
        Chainer tracer, Item parent, ListenableFutureCallback<T> delegate) {
      this.tracer = tracer;
      this.parent = parent;
      this.delegate = delegate;
    }
