...
public class TEServ implements ExecutorService {
  ...
  public void execute(Runnable command) {
    final Runnable r = new LCTRun(tracer(), traceKeys(), spanNamer(), command, this.spanName);
  ...
  public <T> Future<T> submit(Callable<T> task) {
    Callable<T> c = new SCTCall<>(tracer(), traceKeys(), spanNamer(), this.spanName, task);
    return this.delegate.submit(c);
  }

  @Override
  public <T> Future<T> submit(Runnable task, T result) {
    Runnable r = new SCTRun(tracer(), traceKeys(), spanNamer(), task, this.spanName);
    return this.delegate.submit(r, result);
  }

  @Override
  public Future<?> submit(Runnable task) {
    Runnable r = new LCTRun(tracer(), traceKeys(), spanNamer(), task, this.spanName);
  ...
  private <T> Collection<? extends Callable<T>> wrapCallableCollection(
    ...
    for (Callable<T> task : tasks) {
      if (!(task instanceof SCTCall)) {
        ts.add(new SCTCall<>(tracer(), traceKeys(), spanNamer(), this.spanName, task));
      }
    }
    return ts;
  }
