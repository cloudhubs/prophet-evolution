...
public class ChainMetricsAtcfg {
  ...
  protected static class CounterServiceSpanReporterConfig {
    ...
    public ItemMetricReporter spanReporterCounterService(
        CounterService counterService, SMProp sleuthMetricProperties) {
      return new CSBSMRep(
          sleuthMetricProperties.getSpan().getAcceptedName(),
          sleuthMetricProperties.getSpan().getDroppedName(),
          counterService);
    }
  ...
  static class PickMetricIfMetricsIsMissing extends SpringBootCondition {
    ...
    public ConditionOutcome getMatchOutcome(
      ...
      Boolean oldValue =
          context
              .getEnvironment()
              .getProperty(DEPRECATED_SPRING_SLEUTH_METRICS_ENABLED, Boolean.class);
      Boolean newValue =
          context.getEnvironment().getProperty(SPRING_SLEUTH_METRIC_ENABLED, Boolean.class);
      if (oldValue != null) {
        log.warn(
            "You're using an old version of the metrics property. Instead of using ["
