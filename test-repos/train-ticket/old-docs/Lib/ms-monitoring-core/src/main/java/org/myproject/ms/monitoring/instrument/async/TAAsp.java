...
public class TAAsp {
  ...
  public Object traceBackgroundThread(final ProceedingJoinPoint pjp) throws Throwable {
    Item span = this.tracer.createSpan(ItemNameUtil.toLowerHyphen(pjp.getSignature().getName()));
    this.tracer.addTag(Item.SPAN_LOCAL_COMPONENT_TAG_NAME, ASYNC_COMPONENT);
    this.tracer.addTag(
        this.traceKeys.getAsync().getPrefix() + this.traceKeys.getAsync().getClassNameKey(),
  ...
  public Object traceThreadPoolTaskExecutor(final ProceedingJoinPoint pjp) throws Throwable {
    LTTPTExec executor = new LTTPTExec(this.beanFactory, (ThreadPoolTaskExecutor) pjp.getTarget());
  ...
  private Method getMethod(ProceedingJoinPoint pjp, Object object) {
    ...
    return ReflectionUtils.findMethod(
        object.getClass(), method.getName(), method.getParameterTypes());
  }
}
