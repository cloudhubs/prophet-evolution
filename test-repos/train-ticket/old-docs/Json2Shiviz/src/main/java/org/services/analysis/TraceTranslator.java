...
public class TraceTranslator {
  ...
  // "F:\\gitwork\\fault_replicate\\faults-dingding\\F2\\step2_fault_origin_traces";
  ...
  // "F:\\gitwork\\fault_replicate\\faults-dingding\\F2\\step3_fault_shiviz_traces";
  ...
  public static void main(String[] args) throws JSONException {
    ...
    for (int index1 = 0; index1 < dirList.size(); index1++) {
      ...
      if (matcherCaseNumber.find()) testCaseNumber = Integer.parseInt(matcherCaseNumber.group(1));

      List<File> subDirList =
          getTestCaseDirList(
              String.format("%s\\%s", SRC_DIR, dirName)); // Get fail1,fail2... directory
      ...
      for (int index2 = 0; index2 < subDirList.size(); index2++) {
        ...
        for (int x = 0; x < fileLists.size(); x++) {
          ...
          for (int j = 0; j < traceobj.length(); j++) {
            ...
            if (content.get("serverName") != null
                && (content.get("classname") != null || content.get("methodname") != null)) {
              content.put(
                  "api",
                  content.get("serverName")
                      + "."
                      + content.get("classname")
                      + "."
                      + content.get("methodname"));
            } else if (content.get("hostId") != null
                && (content.get("class") != null || content.get("method") != null)) {
              content.put(
                  "api",
          ...
          HashMap<List<HashMap<String, String>>, Boolean> failures =
              new HashMap<List<HashMap<String, String>>, Boolean>();
          traceIds.forEach(
              (n, s) -> {
                List l =
                    logs.stream()
          ...
          writeFile(
              String.format("%s\\shiviz-%s.txt", destDir, fileName),
              listSorted,
              subDirName,
              microserviceName,
              testCaseNumber);
  ...
  private static ArrayList<File> getTestCaseDirList(String path) {
    ...
    for (int i = 0; i < dirList.length; i++) {
      File file = dirList[i];
      if (file.isDirectory()) testDirs.add(file);
    }
    return testDirs;
  }
  ...
  private static List<List<HashMap<String, String>>> sortListByTime(
    ...
    HashMap<String, List<HashMap<String, String>>> traceIdAndList =
  ...
  public static HashMap<String, Integer> findSrcClock(
    ...
    logs.forEach(
        n -> {
          ...
          if (spans.containsKey(spanId)) {
            ...
            Span span =
                new Span(n.get("traceId"), n.get("spanId"), n.get("parentId"), n.get("spanname"));
            span.addLog(n);
  ...
  public static void setChilds(
      HashMap<String, List<String>> spanRelation, Span entrance, HashMap<String, Span> spans) {
    Span s = entrance;
  ...
  public static void traverse(
      Span entrance,
      List<HashMap<String, String>> forwardLogs,
    ...
    if (s.getChilds() != null) {
      ...
      //            List<HashMap<String,String>> childsLogs = new
      ...
      Iterator<String> iterator1 = sortedChilds.iterator();
      List<List<HashMap<String, String>>> childLogLists =
  ...
  private static List<HashMap<String, String>> mergeChildLogLists(
      List<List<HashMap<String, String>>> childLogLists) {
  ...
  private static HashMap<String, String> findEarliest(
      List<HashMap<String, String>> logs, List<List<HashMap<String, String>>> logLists) {
  ...
  */
  private static boolean compareLog(
      HashMap<String, String> log1,
      HashMap<String, String> log2,
      List<HashMap<String, String>> logs1,
      List<HashMap<String, String>> logs2) {
  ...
  public static List<HashMap<String, String>> mergeForwardAndBackwardLogs(
      List<HashMap<String, String>> forwardLogs, List<HashMap<String, String>> backwardLogs) {
    Stack<HashMap<String, String>> stack = new Stack<HashMap<String, String>>();
    backwardLogs.forEach(
        n -> {
          stack.push(n);
        });
  ...
  public static List<HashMap<String, String>> clock2(List<HashMap<String, String>> logs) {
    HashMap<String, HashMap<String, Integer>> clocks =
          ...
          if (clocks.containsKey(n.get("host"))) {
            ...
            allClocks.add(
                new Clock(
                    ...
                    n.get("spanId"),
                    n.get("parentId"),
                    (HashMap<String, Integer>) clock.clone()));
          } else {
  ...
  public static boolean writeFile(
      String path,
      List<List<HashMap<String, String>>> logs,
      String dirName,
      String microserviceName,
      int testCaseNumber) {
    File writer = new File(path);
    ...
    try {
      ...
      if (matcherSuccess.find()) failed = false;
        ...
        while (iterator.hasNext()) {
          ...
          if (map.containsKey("clock")) {
            ...
            for (int i = 0, length = c.length; i < length; i++) {
              c[i] =
                  "\""
                      + c[i].substring(1, c[i].lastIndexOf("="))
                      + "\":"
